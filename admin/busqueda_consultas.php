...
$a_eventos=array('materias', 'docentes', 'secciones','periodos', 'estudiante', 'materia', 'nota_estudiante');
	...
	switch($_POST['oper']){
		...
		case 'periodos':
			if(!isset($_POST['cod_mat'], $_POST['cod_prof']) || empty($_POST['cod_mat']) || empty($_POST['cod_prof']))
				break;
			$sql="SELECT n.cod_per, p.des_per FROM notas AS n LEFT OUTER JOIN periodo AS p ON p.cod_per=n.cod_per WHERE n.cod_mat='".$_POST['cod_mat']."' AND n.cod_prof=".$_POST['cod_prof']." GROUP BY n.cod_per";
			$consulta= mysql_query($sql, $conexion) or die($sql);
			if(!$consulta){
				echo"Error de Consulta", mysql_errno(), mysql_error();
				exit;
		...
		break;
		case 'secciones':
			if(!isset($_POST['cod_comp'], $_POST['cod_prof']) || empty($_POST['cod_comp']) || empty($_POST['cod_prof']) )
				break;
			$sql="SELECT n.cod_mat FROM notas AS n LEFT OUTER JOIN materia AS m ON m.cod_comp=n.cod_comp WHERE n.cod_comp=".$_POST['cod_comp']." AND n.cod_prof=".$_POST['cod_prof']." GROUP BY n.cod_mat";
			$consulta= mysql_query($sql, $conexion) or die($sql);
			if(!$consulta){
				echo"Error de Consulta", mysql_errno(), mysql_error();
				exit;
		...
		break;
		case 'materias':
			$sql="SELECT cod_comp, nom_mat, uc_mat FROM materia ORDER BY nom_mat ASC";
			$consulta= mysql_query($sql, $conexion) or die($sql);
			if(!$consulta){
				echo"Error de Consulta", mysql_errno(), mysql_error();
				exit;
